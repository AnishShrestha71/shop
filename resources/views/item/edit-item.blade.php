...
<div class="grid mt-24 place-items-center">
    <div class="w-11/12 p-12 bg-white sm:w-8/12 md:w-1/2 lg:w-5/12">
        <h1 class="text-xl font-semibold">Hello there 👋, <span class="font-normal">please fill in your information to
                continue</span></h1>
        <form class="mt-6" method="POST" action="{{ route('item.update', $item->id) }}">
            @csrf
            @method('PUT')
            <label for="Name" class="block text-xs font-semibold text-gray-600 uppercase">Name</label>
            <input id="name" type="text" name="name" placeholder="Name" autocomplete="given-name" value="{{$item->name}}"
            ...
            <input id="quantity" type="text" name="quantity" placeholder="quantity" autocomplete="quantity" value="{{$item->quantity}}"
                class="block w-full p-3 mt-2 text-gray-700 bg-gray-200 appearance-none focus:outline-none focus:bg-gray-300 focus:shadow-inner"
                required />
            @error('quantity')
                <div class="text-red-800">{{ $message }}</div>
            ...
            <input id="price" type="text" name="price" placeholder="price" autocomplete="price" value="{{$item->price}}"
                class="block w-full p-3 mt-2 text-gray-700 bg-gray-200 appearance-none focus:outline-none focus:bg-gray-300 focus:shadow-inner"
                required />
            @error('price')
                <div class="text-red-800">{{ $message }}</div>
